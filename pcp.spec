...
Patch1: errata201522154.patch
Patch2: bz1295801.patch
Patch3: bz1297822.patch

# Please note; Patch4 differs slightly from the upstream commit as
# the second (trailing) context line in qa/GNUmakefile drops the 
# 'sheet' SUBDIR
Patch4: bz1223311.patch
...
%if !%{disable_papi}
	%ifarch %{ix86} ppc
		%global _with_papi --with-papi=/usr/lib/papi-5.1.1/usr
	%else
		%global _with_papi --with-papi=/usr/lib64/papi-5.1.1/usr
	%endif
%endif
...
- Fix issue with pmdagfs2 installation (BZ 1305939)
- Address testcase 370 failure on non x86_64 archs (BZ 1305940)
- Address endian issue with pmdalinux on some platforms (BZ 1298722)

* Fri Jan 15 2016 Lukas Berk <lberk@redhat.com> 3.10.9-5
- Update patch to include testcase (BZ 1297822)

* Thu Jan 14 2016 Lukas Berk <lberk@redhat.com> - 3.10.9-4
- Correct interrupts parsing on s390x platforms (BZ 1297822)
- Correct papi version we link against for 32 bit arches (BZ 1295694)

* Thu Jan 07 2016 Lukas Berk <lberk@redhat.com> - 3.10.9-3
- Correct papi version we link against (BZ 1295694)
- Fix possible overflow with pmdaproc using local context options (BZ 1295801)
- Dropped pcp-compat package from upstream (BZ 1294390)

* Thu Dec 17 2015 Lukas Berk <lberk@redhat.com> - 3.10.9-2
- Include post-release platform-defs patch
 
* Wed Dec 16 2015 Lukas Berk <lberk@redhat.com> - 3.10.9-1
- Resolve local_sock() double free error (BZ 1258846).
- Resolve netfilter pmda uses outdated /proc API (BZ 1221653).
- Update to latest PCP sources (BZ 1248272).
- Fix pmatop attribute error (BZ 1195429).
- Fix pmwebd fails when monitoring systems with Vector (BZ 1238863).

* Mon May 04 2015 Nathan Scott <nathans@redhat.com> - 3.10.3-3
- Fix postfix pmda failing to setgid to postfix user (BZ 1213724).
- Fix postfix pmda not reporting incoming mails (BZ 1213764).
- Consistent naming for tools related to LVM cache (BZ 1218007).

* Thu Mar 19 2015 Nathan Scott <nathans@redhat.com> - 3.10.3-2
- Resolve PAPI build dependency issues again (BZ 1188045).
- Fix Avahi-related pmwebd memory corruption (BZ 1199052).
- Resolve QA test case failures (BZ 1198031 & BZ 1198981).

* Mon Mar 02 2015 Nathan Scott <nathans@redhat.com> - 3.10.3-1
- Update to latest PCP sources again (BZ 1158681).
- Fixes bug in s390x /proc/cpuinfo parsing (BZ 1196540).
- Added explicit PAPI 5.1.1 build dependencies (BZ 1188045).
- Improved diagnostics for chkconfig-driven service (BZ 1131022).
- New sub-package for pcp-import-ganglia2pcp.

* Thu Feb 05 2015 Nathan Scott <nathans@redhat.com> - 3.10.2-1
- Update to latest PCP sources (BZ 1158681, BZ 1177064).

* Wed Aug 06 2014 Nathan Scott <nathans@redhat.com> - 3.9.4-5
- Further pcp-testsuite fixes for QE work (BZ 1115953).

* Tue Jul 15 2014 Nathan Scott <nathans@redhat.com> - 3.9.4-4
- Backport pcp-testsuite fixes for QE work (BZ 1115953).

* Thu May 29 2014 Nathan Scott <nathans@redhat.com> - 3.9.4-3
- Backport PCP daemon signal handling fixes (BZ 847343).
- Resolve potential signed integer overflow (ET rpmdiff 97915).
- Update spec to ensure tutorial files live in pcp-doc package.

* Fri May 16 2014 Nathan Scott <nathans@redhat.com> - 3.9.4-2
- Enabled libmicrohttpd use in the build (for PCP Web API).
